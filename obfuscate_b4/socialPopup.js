...
function selectClicked(index) {
    ...
    if (windowOpener) {
        ...
        if (type == 'msg') {
            var replyTextarea = windowOpener.$('#reply-textarea-' + presentTicketId);
...
function uploadTpFile(campaign, whatsappNo, index, tpPropsArr, input) {
    ...
    $.ajax({
        ...
        if (!/^success$/i.test(response.result || "")) {
            ...
            //fileDetails.FileUrl = 'http://' + fileDetails.FileUrl;        // Using http protocol is insecure. Use https instead.
