...
(function ($) {
  ...
  $.fn.idleTimeout = function (userRuntimeConfig) {
    ...
    this.iframeRecheck = function (theDocument) {
      checkForIframes(theDocument);
    };

    this.addPopupIdle = function (theDocument) {
      checkForPopup(theDocument);
    };
    ...
    // document must be in readyState 'complete' before checking for iframes - $(document).ready() is not good enough!
    checkForPopup = function (theDocument) {
      var docReadyCheck, isDocReady;

      docReadyCheck = function () {
        if (document.readyState === "complete") {
          clearInterval(isDocReady);
          includePopup(theDocument);
        }
      };
    ...
    attachEventPopup = function (popupItem) {
      ...
      var iframeContents = popupItem, storeActivityEvents = store.get('activityEvents');
    ...
    includePopup = function (popupContent) {
      if (popupContent.hasClass('jit-inspected') === false) {
        // attach event code for most modern browsers
        popupContent.on('load', attachEventPopup(popupContent)); // Browser NOT IE < 11

        // attach event code for older Internet Explorer browsers
        var domElement = popupContent[0]; // convert jquery object to dom element

        if (domElement.attachEvent) { // Older IE Browser < 11
          domElement.attachEvent('onload', attachEventPopup(popupContent));
        }
      }
    }
