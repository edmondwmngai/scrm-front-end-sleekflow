(function () {
	...
	GaugeChartHelper.prototype.setup = function(chart, config) {
		...
		if (config.options.middleAddUnit) {
			this.middleAddUnit = config.options.middleAddUnit;
		}
	};
	...
	GaugeChartHelper.prototype.renderValueLabel = function() {
		...
		if(this.middleAddUnit){
			ctx.fillText(label, (this.gaugeCenterX - (stringWidth/2)), this.gaugeCenterY);
			ctx.textAlign = "center";
			ctx.font = (fontSizeToUse*0.2) + "px " + this.fontStyle;
			ctx.fillStyle = this.data.valueColor || this.fontColor;
			ctx.fillText(('  ' + this.middleAddUnit), (this.gaugeCenterX + (stringWidth*3)), this.gaugeCenterY);
		}else{
			ctx.fillText(label, this.gaugeCenterX, this.gaugeCenterY); // original
		}	
	};
	...
	Chart.controllers.tsgauge = Chart.controllers.doughnut.extend({
		initialize: function(chart) {
			...
			chart.config.options.animation.onComplete = function(chartElement) {
				...
				// gaugeHelper.animateArrow(); as the gauge will update and don't want start from 0 again
			};
		...
		draw: function() {
			...
			gaugeHelper.renderValueArrow() // commented line above and added this as the gauge will update and don't want start from 0 again
		}
	});
})();
