...
function loadCampaignTbl() {
    $.ajax({
        ...
        contentType: "application/json",
        dataType: 'json'
    }).always(handleCampaignResponse);
}

function handleCampaignResponse(r) {
    var rDetails = r.details;

    // Handle case where form does not exist
    if (rDetails == 'form does not exist') {
        r.result = 'success';
        rDetails = [];
    }
    ...
    inputFormData = rDetails;
    loadBatchTbl(); // batch table need to have campaign name (inputFormData)
    updateCampaignTable(rDetails);
    updateCampaignSelectOptions(rDetails);
    $('.l-campaign-select').text(langJson['l-campaign-select']);
}

function updateCampaignTable(rDetails) {
    if (cInputFormTbl) {
        // Update existing DataTable
        $('#c-campaign-tbl').DataTable().clear();
        $('#c-campaign-tbl').DataTable().rows.add(rDetails);
        $('#c-campaign-tbl').DataTable().columns.adjust().draw();
    } else {
        ...
        cInputFormTbl = $('#c-campaign-tbl').DataTable({
            ...
            columnDefs: [{
                ...
                data: null,
                defaultContent:
                    '<button class="btn btn-sm rounded btn-warning text-capitalize">' +
                    '<i class="fas fa-mouse-pointer me-2"></i>' +
                    '<span class="l-campaign-select">Select</span></button>',
                className: 'btnColumn',
                orderable: false
            }],
            ...
            columns: [
                ...
                { title: langJson['l-account-name'], data: "Form_Name" },
                { title: langJson['l-form-details'], data: "Form_Details" }
            ]
        });

        // Input Form Select Event
        $('#c-campaign-tbl tbody').on('click', 'button', handleCampaignSelect);
    }
}
...
function handleCampaignSelect() {
    ...
    $('#c-campaign-tbl').DataTable().$('tr.highlight').removeClass('highlight');
    $row.addClass('highlight');
    $('#c-select-channel').removeClass('d-none');
    // window.scrollTo(0,document.body.scrollHeight); // commented as per original
}

function updateCampaignSelectOptions(rDetails) {
    $('#o-search-campaign-select').empty();
    var optionsStr = '<option value="">- Please Select -</option>';
    for (let formObj of rDetails) {
        optionsStr += `<option value="${formObj.Form_Id}">${formObj.Form_Name}</option>`;
    }
    $('#o-search-campaign-select').append(optionsStr);
}
...
function loadBatchTbl() {
    $.ajax({
        ...
        if (!/^success$/i.test(r.result || "")) {
			...
			processLoadBatchTblData(rDetails)
		}
    })
}

function processLoadBatchTblData(rDetails)
{
		...
		{
			processBatchTblDataDetail(batchTblData);
			
		}
			
}            

function processBatchTblDataDetail(batchTblData)
{
        $('#left-menu').removeClass('d-none');
        batchTbl = $('#m-batch-tbl').DataTable({
            data: batchTblData,
        ...
        $('#m-batch-tbl tbody').on('click', '.m-call', function () {
            ...
            $.ajax({
                ...
                if (!/^success$/i.test(r.result || "")) {
                    ...
                    processBatchTblCall(campaignDetails, data, batchId, loginId, token, selectedBtn)
                }
            });
        });
...
function processBatchTblCall(campaignDetails, data, batchId, loginId, token, selectedBtn)
{
    // Add selected table html
    // Add Title 
    var selectTblHtml = '<div class="card my-2"><div class="card-body pt-0 bg-lightgray"><div class="c-section"><div class="my-1"><h5 class="d-inline l-campaign-selected-channel">Selected Campaign & Channel</h5></div>';
    ...
    $.ajax({
        ...
        if (!/^success$/i.test(r.result || "")) {
            ...
            // Add From Select
            var unassignment = data.Unassigned;
            lowerPartStr += '<div class="card my-2"><div class="card-body"><div class="d-table"><div class="mb-3 d-flex align-items-center d-table-row mb-1"><span class="form-label pe-3 d-table-cell l-campaign-from">Assign From</span><select id="m-assign-from" class="form-select d-table-cell"><option value=0 unused=' + unassignment + '>Pool</option>' + agentOptionStr + '</select><span id="m-from-description" class="text-secondary d-table-cell ps-2">Unassignment: ' + unassignment + '</span><span class=""></div>';   //remove from-control blank
            // Add Optional Call Result
            lowerPartStr += '<div id="m-call-result-container" class="mb-3 d-flex align-items-center mb-1 d-none"><span class="d-table-cell me-2 l-campaign-callstatus">Call Status&nbsp;&nbsp;</span><select id="m-call-result" class="form-select d-table-cell"><option value="NewLead" selected>Null</option><option value="Unreach">Unreach</option><option value="Reached">Reached</option><option value="Successful Order">Successful Order</option></select>' +
...
$(document).ready(function () {
    ...
    $('#o-search-btn').on('click', function (event) {
        ...
        if (customerTbl != null) {
            ...
            //20250530 for Refactor this code to not nest functions more than 4 levels deep.
            loadCustomerTbl(loginId, token);
            /*
...
function loadCustomerTbl(loginId, token)
{
  customerTbl = $('#o-search-results-tbl').DataTable({
                "serverSide": true,
				"ajax": {
