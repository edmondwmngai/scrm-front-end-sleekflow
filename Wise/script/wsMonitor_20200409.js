...
var wsWiseMonitor = {
	...
	OpenWebsocket: function(serverName, callback) {
        if ("WebSocket" in window) {
            ...
            wsRM = new WebSocket((config.isHttps? "ws://": "ws://") + serverName +":2312");
                ...
                switch (obj.Command) {
						...
						switch(obj.ObjectType)
						{
							...
							*/
							...
							if(i==-1){
								...
								"agentStatus": wsWiseMonitor.arrAgentStatus[parseInt(obj.StatusID)], "callId" : parseInt(obj.CallID), "serviceName": ""});
								...
								if(wsWiseMonitor.AgentList[i].agentName==""){
									...
									if(wsWiseMonitor.AgentList[i].statusId!=5 && wsWiseMonitor.AgentList[i].statusId!=6) wsWiseMonitor.AgentList[i].serviceName="";
									wsWiseMonitor.triggerEvent(document, 'onRMAgentInfo', wsWiseMonitor.AgentList[i]);
									console.log(wsWiseMonitor.AgentList);
					...
					case "6":	//Object Info
						...
						switch(obj.ObjectType)
						{
							...
							break;
							case "20":
								var i= wsWiseMonitor.AgentList.findIndex(function (v) { return v.callId==parseInt(obj.CallID)});
								
								if(i!=-1){
									console.log(wsWiseMonitor.AgentList[i]);
